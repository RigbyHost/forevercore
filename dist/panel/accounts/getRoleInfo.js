 ...
 */
async function getRoleInfo(accountID) {
    try {
        const db = await (0, db_1.default)('main');
        // Получаем ID роли из таблицы назначения ролей
        const [roleAssignRows] = await db.execute('SELECT roleID FROM roleassign WHERE accountID = ?', [accountID]);
        // Если роль не назначена, возвращаем роль по умолчанию
        if (roleAssignRows.length === 0) {
            console_api_1.default.Log("main", `No role assigned for accountID: ${accountID}, using default role`);
            return DEFAULT_ROLE;
        }
        const roleID = roleAssignRows[0].roleID;
        // Получаем данные роли из таблицы ролей
        const [roleRows] = await db.execute('SELECT roleName, advancedPanel, adminPanel FROM roles WHERE roleID = ?', [roleID]);
        // Если роль не найдена, возвращаем роль по умолчанию
        if (roleRows.length === 0) {
            console_api_1.default.Warn("main", `Role ID ${roleID} not found for accountID: ${accountID}, using default role`);
            return DEFAULT_ROLE;
        }
        // Возвращаем информацию о роли
        const { roleName, advancedPanel, adminPanel } = roleRows[0];
        console_api_1.default.Log("main", `Panel action: received role info. accountID: ${accountID}, roleName: ${roleName}`);
        return {
            roleName,
            advancedPanel,
            adminPanel
        ...
        console_api_1.default.Error("main", `Error retrieving role info for accountID ${accountID}: ${error}`);
 ...
 */
async function getAllUserRoles(accountID) {
    try {
        const db = await (0, db_1.default)('main');
        ...
        const [roleRows] = await db.execute(`SELECT roleName, advancedPanel, adminPanel FROM roles 
       WHERE roleID IN (${roleIDs.map(() => '?').join(',')})`, roleIDs);
        ...
        console_api_1.default.Error("main", `Error retrieving all roles for accountID ${accountID}: ${error}`);
        // В случае ошибки возвращаем массив с ролью по умолчанию
        return [DEFAULT_ROLE];
    }
}
exports.default = getRoleInfo;
