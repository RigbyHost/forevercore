...
const getComments = async (binaryVersionStr, gameVersionStr, modeStr, countStr, pageStr, levelIDStr, userIDStr) => {
    try {
        ...
        const users = new Set();
        // Parse parameters
        const binaryVersion = binaryVersionStr ? await exploitPatch_1.default.remove(binaryVersionStr) : '0';
        const gameVersion = gameVersionStr ? await exploitPatch_1.default.remove(gameVersionStr) : '0';
        const mode = modeStr ? await exploitPatch_1.default.remove(modeStr) : '0';
        const count = countStr && !isNaN(Number(countStr)) ? await exploitPatch_1.default.remove(countStr) : '10';
        const page = pageStr ? await exploitPatch_1.default.remove(pageStr) : '0';
        ...
        if (levelIDStr) {
            ...
            filterID = await exploitPatch_1.default.remove(levelIDStr);
            ...
            displayLevelID = true;
            filterID = await exploitPatch_1.default.remove(userIDStr);
        ...
        // Get comment count
        const [countResult] = await db_proxy_1.default.query(`SELECT count(*) as count FROM comments ${userListJoin} WHERE ${filterToFilter}${filterColumn} = ? ${userListWhere}`, [filterID]);
        const commentcount = countResult[0].count;
        if (commentcount == 0) {
            return "-2";
        }
        // Get comments
        const [result] = await db_proxy_1.default.query(`SELECT comments.levelID, comments.commentID, comments.timestamp, comments.comment, 
            ...
            LIMIT ? OFFSET ?`, [filterID, parseInt(count), commentpage]);
        ...
        for (const comment1 of result) {
            if (comment1.commentID) {
                // Format date
                const uploadDate = new Date(comment1.timestamp * 1000).toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })
                    .replace(/,/g, "")
                    .replace(/:/g, ".");
                ...
                const commentParts = [];
                if (displayLevelID)
                    commentParts.push(`1~${comment1.levelID}`);
                ...
                if (comment1.userName) {
                    ...
                    if (typeof binVer === 'number' && binVer > 31) {
                        ...
                        const badge = await apiLib_1.default.getMaxValuePermission(extID, "modBadgeLevel");
                        // Check if badge is a number and greater than 0
                        const badgeValue = typeof badge === 'number' && badge > 0;
                        const colorString = badgeValue ? `~12~${await apiLib_1.default.getAccountCommentColor(extID)}` : "";
        ...
        console_api_1.default.Log("main", `Received ${commentcount} comments, page ${commentpage}`);
        return response;
    }
    catch (error) {
        console_api_1.default.Error("main", `${error} at net.fimastgd.forevercore.api.comments.getComments`);
        return "-1";
    }
};
