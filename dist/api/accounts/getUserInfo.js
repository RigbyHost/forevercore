 ...
 */
const getUserInfo = async (gdpsid, targetAccountIDStr, accountIDStr, gjp2Str, gjpStr, req) => {
    try {
        const db = await (0, db_1.default)(gdpsid);
        let appendix = "";
        let me;
        const extid = await exploitPatch_1.default.number(targetAccountIDStr);
        // Authenticate user
        if (gjp2Str) {
            me = accountIDStr ? await GJPCheck_1.default.getAccountIDOrDie(accountIDStr, gjp2Str, undefined, req) : 0;
        }
        else {
            me = accountIDStr ? await GJPCheck_1.default.getAccountIDOrDie(accountIDStr, undefined, gjpStr, req) : 0;
        }
        // Check if blocked
        ...
        if (rows[0].count > 0) {
            console_api_1.default.Error("main", `Failed to get user info. ID: ${extid}`);
            return "-1";
        }
        // Get user data
        ...
        let badge = await apiLib_1.default.getMaxValuePermission(extid, "modBadgeLevel");
        ...
        console_api_1.default.Log("main", `Received user info. ID: ${extid}`);
        ...
        console_api_1.default.Error("main", `${err} at net.fimastgd.forevercore.api.accounts.getUserInfo`);
        return "-1";
    }
};
