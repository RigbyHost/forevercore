...
const getLevels = async (gameVersionStr, binaryVersionStr, typeStr, diffStr, uncompletedStr, originalStr, coinsStr, completedLvlsStr, onlyCompletedStr, songStr, customSongStr, twoPlayerStr, starStr, noStarStr, gauntletStr, lenStr, featuredStr, epicStr, mythicStr, legendaryStr, demonFilterStr, strStr, pageStr, followedStr, accountIDStr, gjpStr, gjp2Str, req) => {
    try {
        ...
        let params = ["unlisted = 0"];
        let morejoins = "";
        // Process input parameters
        let gameVersion, binaryVersion, type, diff;
        let completedLevels, song, gauntlet, epicFilter;
        let offset, uploadDate, followed, accountID, peoplearray, whereor;
        let listLevels, totallvlcount;
        let sug = "", sugg = "";
        let countquery;
        // Parse game version
        if (gameVersionStr) {
        ...
        if (uncompletedStr && uncompletedStr === "1") {
            completedLevels = await exploitPatch_1.default.numbercolon(completedLvlsStr);
        ...
        if (songStr) {
            if (!customSongStr) {
                song = parseInt(await exploitPatch_1.default.number(songStr)) - 1;
                params.push(`audioTrack = ${song} AND songID = 0`);
            }
            else {
                song = parseInt(await exploitPatch_1.default.number(songStr));
        ...
        // Gauntlet filter
        if (gauntletStr) {
            gauntlet = await exploitPatch_1.default.remove(gauntletStr);
            const [rows] = await db_proxy_1.default.execute("SELECT * FROM gauntlets WHERE ID = ?", [gauntlet]);
        ...
        // Length filter
        const len = lenStr ? await exploitPatch_1.default.numbercolon(lenStr) : "-";
        if (len !== "-" && len) {
            params.push(`levelLength IN (${len})`);
        }
        // Rating filters
        if (featuredStr && featuredStr !== "0")
            epicParams.push("starFeatured = 1");
        if (epicStr && epicStr !== "")
            epicParams.push("starEpic = 1");
        if (mythicStr && mythicStr !== "")
            epicParams.push("starEpic = 2");
        if (legendaryStr && legendaryStr !== "")
            epicParams.push("starEpic = 3");
        epicFilter = epicParams.join(" OR ");
        if (epicFilter && epicFilter !== "")
            params.push(epicFilter);
        // Difficulty filters
        if (diff !== undefined) {
        ...
        // Search string
        if (strStr) {
            str = await exploitPatch_1.default.remove(strStr);
        }
        // Pagination
        if (pageStr && !isNaN(parseInt(pageStr))) {
            const patch = await exploitPatch_1.default.number(pageStr);
            offset = `${patch}0`;
        }
        else {
            offset = "0";
        ...
        switch (type) {
            ...
            case 23: // EVENT SAFE (assumption)
                ...
                break;
            case 25: // LIST LEVELS
                listLevels = await apiLib_1.default.getListLevels(str);
        ...
        // Execute queries
        const [countResult] = await db_proxy_1.default.query(countquery);
        totallvlcount = countResult[0].total;
        const [result] = await db_proxy_1.default.execute(finalQuery);
        ...
        for (const level1 of result) {
            if (level1.levelID) {
                ...
                if (isIDSearch && level1.unlisted > 1) {
                    ...
                    if (!(await apiLib_1.default.isFriends(accountID, level1.extID)) && accountID != level1.extID) {
                        break;
                    }
                }
                ...
                if (level1.songID !== 0) {
                    const song = await apiLib_1.default.getSongString(level1);
                    if (song) {
                        songsstring += `${song}~:~`;
                    }
                }
                // Add user info
                userstring += `${await apiLib_1.default.getUserString(level1)}|`;
            }
        }
        ...
        resultFN += await generateHash_1.default.genMulti(lvlsmultistring);
        console_api_1.default.Log("main", `Received levels by accountID: ${accountIDStr || "unknown"}`);
        return resultFN;
    }
    catch (error) {
        console_api_1.default.Error("main", `${error} at net.fimastgd.forevercore.api.levels.getLevels`);
        return "-1";
    }
};
exports.default = getLevels;
