...
class ApiLib {
    ...
    static getIP(req) {
        if (!req || !req.headers) {
            console_api_1.default.Error("main", "ERROR WITH IP() HEADERS!!! UNDEFINED!");
    ...
    static async getMaxValuePermission(accountID, permission) {
        ...
        let maxValue = 0;
        try {
            const [roleIDarray] = await db_proxy_1.default.execute("SELECT roleID FROM roleassign WHERE accountID = ?", [accountID]);
            if (roleIDarray.length === 0)
                return maxValue;
            const roleIDlist = roleIDarray.map(row => row.roleID).join(",");
            const [roles] = await db_proxy_1.default.execute(`SELECT ${permission} FROM roles WHERE roleID IN (${roleIDlist}) ORDER BY priority DESC`);
            roles.forEach(role => {
            ...
            console_api_1.default.Error("main", `Error fetching permission data: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return false;
        }
    }
    /**
    ...
    static async getIDFromPost(udidStr, gameVersionStr, accountIDStr, gjp2Str, gjpStr, req) {
        try {
            if (gjpStr) {
                if (udidStr && gameVersionStr && parseInt(gameVersionStr) < 20 && settings_1.settings.unregisteredSubmissions) {
                    const id = await exploitPatch_1.default.remove(udidStr);
            ...
            console_api_1.default.Error("main", `getIDFromPostException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return "-1";
        }
    }
    /**
    ...
    static async getUserID(extID, userName = "Undefined") {
        try {
            ...
            const [rows] = await db_proxy_1.default.execute("SELECT userID FROM users WHERE extID = ?", [extID]);
            if (rows.length > 0) {
                return rows[0].userID;
            }
            else {
                const time = Math.floor(Date.now() / 1000);
                const [result] = await db_proxy_1.default.execute("INSERT INTO users (isRegistered, extID, userName, lastPlayed) VALUES (?, ?, ?, ?)", [register, extID, userName, time]);
                return result.insertId;
            }
        }
        catch (error) {
            console_api_1.default.Error("main", `getUserIDException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return "-1";
        }
    }
    /**
    ...
    static async checkPermission(accountID, permission) {
        try {
            // Check if admin
            const [adminRows] = await db_proxy_1.default.execute("SELECT isAdmin FROM accounts WHERE accountID = ?", [accountID]);
            if (adminRows.length > 0 && adminRows[0].isAdmin === 1) {
                return true;
            }
            // Check role permissions
            const [roleRows] = await db_proxy_1.default.execute("SELECT roleID FROM roleassign WHERE accountID = ?", [accountID]);
            if (roleRows.length > 0) {
                const roleIDs = roleRows.map(row => row.roleID).join(",");
                const [permRows] = await db_proxy_1.default.execute(`SELECT ${permission} FROM roles WHERE roleID IN (${roleIDs}) ORDER BY priority DESC`);
            ...
            const [defaultRows] = await db_proxy_1.default.execute(`SELECT ${permission} FROM roles WHERE isDefault = 1`);
            ...
            console_api_1.default.Error("main", `checkPermissionException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return false;
        }
    }
    /**
    ...
    static async getFriends(accountID) {
        try {
            const [rows] = await db_proxy_1.default.execute("SELECT person1, person2 FROM friendships WHERE person1 = ? OR person2 = ?", [accountID, accountID]);
            if (rows.length === 0)
                return [];
            return rows.map(row => {
            ...
            console_api_1.default.Error("main", `getFriendsException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return [];
        }
    }
    /**
    ...
    static async getListLevels(listID) {
        try {
            const [rows] = await db_proxy_1.default.execute("SELECT listlevels FROM lists WHERE listID = ?", [listID]);
            return rows.length > 0 ? rows[0].listlevels : null;
        }
        catch (error) {
            console_api_1.default.Error("main", `getListLevelsException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return null;
        }
    }
    /**
    ...
    static async isFriends(accountID, targetAccountID) {
        try {
            const [rows] = await db_proxy_1.default.execute("SELECT COUNT(*) as count FROM friendships WHERE (person1 = ? AND person2 = ?) OR (person1 = ? AND person2 = ?)", [accountID, targetAccountID, targetAccountID, accountID]);
            return rows[0].count > 0;
        }
        catch (error) {
            console_api_1.default.Error("main", `isFriendsException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return false;
        }
    }
    /**
     ...
     */
    static async getDiffFromStars(stars) {
        try {
            let auto = 0;
            let demon = 0;
            let diffname;
            ...
            console_api_1.default.Error("main", `getDiffFromStarsException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
    ...
    static async rateLevel(accountID, levelID, stars, difficulty, auto, demon) {
        try {
            ...
            await db_proxy_1.default.execute("UPDATE levels SET starDemon=?, starAuto=?, starDifficulty=?, starStars=?, rateDate=? WHERE levelID=?", [demon, auto, difficulty, stars, timestamp, levelID]);
            const diffName = await this.getDiffFromStars(stars);
            await db_proxy_1.default.execute("INSERT INTO modactions (type, value, value2, value3, timestamp, account) VALUES (?, ?, ?, ?, ?, ?)", ['1', diffName.name, timestamp, stars, levelID, accountID]);
        }
        catch (error) {
            console_api_1.default.Error("main", `rateLevelException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
        }
    }
    /**
    ...
    static async getListOwner(listID) {
        try {
            const [rows] = await db_proxy_1.default.execute("SELECT accountID FROM lists WHERE listID = ?", [listID]);
            return rows.length > 0 ? rows[0].accountID : null;
        }
        catch (error) {
            console_api_1.default.Error("main", `getListOwnerException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return null;
        }
    }
    /**
    ...
    static async getAccountIDFromName(userName) {
        try {
            ...
            console_api_1.default.Error("main", `getAccountIDFromNameException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return 0;
        }
    }
    /**
    ...
    static async getListName(listID) {
        try {
            const [rows] = await db_proxy_1.default.execute("SELECT listName FROM lists WHERE listID = ?", [listID]);
            return rows.length > 0 ? rows[0].listName : null;
        }
        catch (error) {
            console_api_1.default.Error("main", `getListNameException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return null;
        }
    }
    /**
    ...
    static async getAccountCommentColor(accountID) {
        try {
            if (await !is.Int(accountID))
                return "255,255,255";
            const [roleRows] = await db_proxy_1.default.execute("SELECT roleID FROM roleassign WHERE accountID = ?", [accountID]);
            if (roleRows.length > 0) {
                const roleIDs = roleRows.map(row => row.roleID).join(",");
                const [colorRows] = await db_proxy_1.default.execute(`SELECT commentColor FROM roles WHERE roleID IN (${roleIDs}) ORDER BY priority DESC`);
            ...
            const [defaultRows] = await db_proxy_1.default.execute("SELECT commentColor FROM roles WHERE isDefault = 1");
            if (defaultRows.length > 0) {
            ...
            console_api_1.default.Error("main", `getAccountCommentColorException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return "255,255,255";
        }
    }
    /**
    ...
    static async suggestLevel(accountID, levelID, difficulty, stars, feat, auto, demon) {
        try {
            await db_proxy_1.default.execute("INSERT INTO suggest (suggestBy, suggestLevelID, suggestDifficulty, suggestStars, suggestFeatured, suggestAuto, suggestDemon, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [accountID, levelID, difficulty, stars, feat, auto, demon, Math.floor(Date.now() / 1000)]);
        }
        catch (error) {
            console_api_1.default.Error("main", `suggestLevelException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
        }
    }
    /**
    ...
    static async verifyCoinsLevel(accountID, levelID, coins) {
        try {
            await db_proxy_1.default.execute("UPDATE levels SET starCoins=? WHERE levelID=?", [coins, levelID]);
            await db_proxy_1.default.execute("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (?, ?, ?, ?, ?)", ['3', coins, levelID, Math.floor(Date.now() / 1000), accountID]);
        }
        catch (error) {
            console_api_1.default.Error("main", `verifyCoinsLevelException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
        }
    }
    /**
    ...
    static async featureLevel(accountID, levelID, state) {
        try {
            ...
            await db_proxy_1.default.execute("UPDATE levels SET starFeatured=?, starEpic=?, rateDate=? WHERE levelID=?", [feature, epic, Math.floor(Date.now() / 1000), levelID]);
            await db_proxy_1.default.execute("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (?, ?, ?, ?, ?)", ['2', state, levelID, Math.floor(Date.now() / 1000), accountID]);
        }
        catch (error) {
            console_api_1.default.Error("main", `featureLevelException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
        }
    }
    /**
    ...
    static async getAccountName(accountID) {
        try {
            if (isNaN(Number(accountID)))
                return false;
            const [rows] = await db_proxy_1.default.execute("SELECT userName FROM accounts WHERE accountID = ?", [accountID]);
            return rows.length > 0 ? rows[0].userName : false;
        }
        catch (error) {
            console_api_1.default.Error("main", `getAccountNameException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return false;
        }
    }
    /**
    ...
    static async check(gjp, accountID, req) {
        if (settings_1.settings.sessionGrants) {
            const ip = fixIp_1.default.getIP(req);
        ...
        if (validationResult === 1 && settings_1.settings.sessionGrants && req) {
    ...
    static async getAccountIDOrDie(accountIDStr, gjp2Str, gjpStr, req) {
        ...
        const accountID = exploitPatch_1.default.remove(accountIDStr);
        if (gjpStr) {
    ...
    static async assignGJP2(accid, pass, req) {
        ...
        try {
            ...
            console_api_1.default.Error("main", `assignGJP2Exception: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return "-1";
        }
    }
    /**
    ...
    static async isGJP2ValidUsrname(userName, gjp2, req) {
        try {
            ...
            console_api_1.default.Error("main", `isGJP2ValidUsrnameException: ${error} at net.fimastgd.forevercore.api.lib.apiLib`);
            return 0;
        }
    }
    /**
