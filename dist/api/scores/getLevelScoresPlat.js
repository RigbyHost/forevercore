...
const getLevelScoresPlat = async (req) => {
    try {
        // Authenticate user
        const accountID = parseInt(await GJPCheck_1.default.getAccountIDOrDie(req.body.accountID, req.body.gjp2, req.body.gjp, req));
        const levelID = await exploitPatch_1.default.remove(req.body.levelID);
        // Get submitted score
        const scores = {
            time: parseInt(exploitPatch_1.default.number(req.body.time)),
            points: await exploitPatch_1.default.number(req.body.points)
        };
        ...
        // UPDATING SCORE
        const [oldScoreRows] = await db_proxy_1.default.query(`SELECT ${mode} FROM platscores WHERE accountID = ? AND levelID = ?`, [accountID, levelID]);
        // Either insert new score or update if better
        if (oldScoreRows.length == 0) {
            if (scores.time > 0) {
                await db_proxy_1.default.query("INSERT INTO platscores (accountID, levelID, time, timestamp) VALUES (?, ?, ?, ?)", [accountID, levelID, scores.time, uploadDate]);
            }
        }
        else {
            ...
            if (((mode == "time" && oldScore > scores.time) ||
                ...
                scores.time > 0) {
                await db_proxy_1.default.query(`UPDATE platscores SET ${mode}=?, timestamp=? WHERE accountID=? AND levelID=?`, [scores[mode], uploadDate, accountID, levelID]);
            }
        }
        // GETTING SCORES
        ...
        // Get scores
        const [scores2] = await db_proxy_1.default.query(query, queryArgs);
        // Format response
        let x = 0;
        for (const score of scores2) {
            const extID = score.accountID;
            // Get user data
            const [userRows] = await db_proxy_1.default.query("SELECT userName, userID, icon, color1, color2, color3, iconType, special, extID, isBanned FROM users WHERE extID = ?", [extID]);
        ...
        console_api_1.default.Log("main", `Received platformer level scores by accountID: ${accountID}`);
        return lvlstr.slice(0, -1); // Remove trailing pipe
    }
    catch (error) {
        console_api_1.default.Error("main", `${error} at net.fimastgd.forevercore.api.scores.getLevelScoresPlat`);
        return "-1";
    }
};
exports.default = getLevelScoresPlat;
