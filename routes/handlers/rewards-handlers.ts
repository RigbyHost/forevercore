...
export class GetChallengesHandler extends BaseApiHandler {
	constructor() {
		super("/:gdpsid/getGJChallenges.php");
	}

	async handle(req: Request, res: Response): Promise<void> {
		try {
			const result = await getChallenges(req.params.gdpsid.toString(), req.body.accountID, req.body.udid, req.body.chk);
...
export function createRewardsHandlers() {
	return [new GetChallengesHandler(), new GetChestsHandler()];
}
