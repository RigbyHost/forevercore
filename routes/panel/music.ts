...
router.get('/list', async (req: express.Request, res: express.Response) => {
    ...
    ConsoleApi.Log("Query thread", `Handled new session '/panel/music/list', opened by ${req.cookies.username}`);
    let offset: number;
    // console.log(req.query.offset)
    offset = 0;
    const page: number = 0;
    const table = await getSongList(offset);
...
router.get('/:route', async (req: express.Request, res: express.Response) => {
    ...
    if (ROUTE === "newgrounds") {
        ...
        ConsoleApi.Log("Query thread", `Handled new session '/panel/music/newgrounds', opened by ${req.cookies.username}`);
        ...
        ConsoleApi.Log("Query thread", `Handled new session '/panel/music/link', opened by ${req.cookies.username}`);
        const data = { GDPS: settings.serverName, GDPSID: settings.GDPSID, captchaKey: captcha.key };
        ...
        ConsoleApi.Log("Query thread", `Handled new session '/panel/music/dropbox', opened by ${req.cookies.username}`);
        const data = { GDPS: settings.serverName, GDPSID: settings.GDPSID, captchaKey: captcha.key };
        ...
        ConsoleApi.Log("Query thread", `Handled new session '/panel/music/youtube', opened by ${req.cookies.username}`);
        const data = { GDPS: settings.serverName, GDPSID: settings.GDPSID, captchaKey: captcha.key };
...
router.post('/link', async (req: express.Request, res: express.Response) => {
    ...
    const result = await getSongLinkInfo(array);
    res.status(200).send(result);
});
...
router.post('/dropbox', async (req: express.Request, res: express.Response) => {
    ...
    SIZE = fileSize !== "Undefined" ? fileSize : "Unknown";
    const array: [string, string, string | number] = [req.body.songname, req.body.songurl, SIZE];
    const result = await getSongDropboxInfo(array);
    res.status(200).send(result);
});
...
router.post('/youtube', async (req: express.Request, res: express.Response) => {
    ...
    const songurl: string = settings.serverURL + `/music/${rstr}.mp3`;
    ...
    async function download(videoUrl: string, GDPSID: string): Promise<void> {
        const outputPath = path.join(__dirname, '..', '..', 'public', 'music', `${rstr}.mp3`);
        ...
        try {      
            await new Promise<void>((resolve, reject) => {
                exec(videoUrl, {
                    ...
                    ConsoleApi.Error("main", `Panel action: failed to download from youtube: ${error} at net.fimastgd.forevercore.routes.panel.music`);
                    reject(error);
                });
            });
        } catch (error) {
    ...
    if (isTimeOverload == 0 && isUnknownSong == 0 && isSongDublicate == false) {
        ...
        const array: [string, string, string | number, string] = [req.body.songname, songurl, SIZE, originalLink];
                // [4]
        // checkMemory();
        result = await getSongYoutubeInfo(array);
                // [5]
        // checkMemory();
