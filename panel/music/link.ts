...
async function getSongLinkInfo(gdpsid: string, array: [string, string, string | number]): Promise<string> {
	...
	try {
		...
		if (songId === -1) {
			...
			const [result] = await db.execute<ResultSetHeader>(query, [null, array[0], 8, "Forever Music [Link]", array[2], array[1]]);
