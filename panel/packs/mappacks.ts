...
async function getLevelName(gdpsid: string, level: number): Promise<string> {
	try {
		const levelName = await Panel.getLevelName(gdpsid, level);
...
async function getMapPacks(gdpsid: string): Promise<FormattedMapPacks[]> {
	...
	const formattedData = await Promise.all(
		rows.map(async row => {
			...
			const levels = await Promise.all(levelsArray.map(level => getLevelName(gdpsid, level)));
