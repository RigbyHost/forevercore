...
async function getGauntlets(gdpsid: string) {
	...
	return Promise.all(
		rows.map(async (row: RowDataPacket) => {
			...
			return {
				...
				level1: `${await Panel.getLevelName(gdpsid, row.level1)} (${row.level1})`,
				level2: `${await Panel.getLevelName(gdpsid, row.level2)} (${row.level2})`,
				level3: `${await Panel.getLevelName(gdpsid, row.level3)} (${row.level3})`,
				level4: `${await Panel.getLevelName(gdpsid, row.level4)} (${row.level4})`,
				level5: `${await Panel.getLevelName(gdpsid, row.level5)} (${row.level5})`,
				originalid: row.ID
			};
		})
	);
