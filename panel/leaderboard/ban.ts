...
async function banUser(gdpsid: string, userName: string): Promise<int> {
	...
	try {
		...
		const [updateResult]: [ResultSetHeader, FieldPacket[]] = await db.execute("UPDATE users SET isBanned = 1 WHERE LOWER(userName) = LOWER(?)", [
			userName
		]);
		...
		await db.execute("INSERT INTO modactions (type, value, value2, timestamp, account) VALUES ('15', ?, '1', ?, ?)", [
			accountID,
			timestamp,
			accountID
		]);
