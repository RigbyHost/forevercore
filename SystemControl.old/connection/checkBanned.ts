...
export async function checkActive(id: string, node: string): Promise<boolean> {
	try {
		...
		let isBanned: boolean = Boolean(props.isBanned);
		// возвращаем Promise<boolean>
		return isBanned; 
	} catch (e) {
		ConsoleApi.Error("SystemControl", `${e} at net.fimastgd.forevercore.SystemControl.SystemControl.connection.checkBanned`);
		return false;
	}
}  