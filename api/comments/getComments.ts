	...
	try {
		...
		for (const comment1 of result) {
			if (comment1.commentID) {
				// Format date
				const uploadDate = new Date(comment1.timestamp * 1000)
				...
				const commentText = gameVer < 20 ? Buffer.from(comment1.comment, "base64").toString() : comment1.comment;
				...
				if (comment1.userName) {
					...
					if (typeof binVer === "number" && binVer > 31) {
						...
						const badge = await ApiLib.getMaxValuePermission(gdpsid, extID, "modBadgeLevel");

						// Check if badge is a number and greater than 0
						const badgeValue = typeof badge === "number" && badge > 0;
						const colorString = badgeValue ? `~12~${await ApiLib.getAccountCommentColor(gdpsid, extID)}` : "";
