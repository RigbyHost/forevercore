...
const uploadComment = async (
	...
	try {
		...
		const userID = await ApiLib.getUserID(gdpsid, id, userName);
		const uploadDate = Math.floor(Date.now() / 1000);
		const decodecomment = Buffer.from(comment, "base64").toString("utf-8");

		// Проверяем, является ли комментарий командой
		const commandResult = await CommandLib.doCommands(gdpsid, id, decodecomment, levelID);
		...
		if (id != "" && comment != "") {
			...
			if (register && parseInt(percent.toString()) != 0) {
				const [rows] = await db.execute<any[]>("SELECT percent FROM levelscores WHERE accountID = ? AND levelID = ?", [id, levelID]);

				if (rows.length == 0) {
					await db.execute("INSERT INTO levelscores (accountID, levelID, percent, uploadDate) VALUES (?, ?, ?, ?)", [
						id,
						levelID,
						percent,
						uploadDate
					...
					await db.execute("UPDATE levelscores SET percent = ?, uploadDate = ? WHERE accountID = ? AND levelID = ?", [
						percent,
						uploadDate,
						id,
						levelID
