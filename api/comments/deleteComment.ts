...
const deleteComment = async (
	...
	try {
		...
		let [result] = await db.execute<ResultSetHeader>("DELETE FROM comments WHERE commentID = ? AND userID = ? LIMIT 1", [commentID, userID]);
		...
		if (result.affectedRows === 0) {
         ...
         WHERE commentID = ?`,
			...
			if (String(creatorAccID) === String(accountID) || (await ApiLib.checkPermission(gdpsid, accountID, "actionDeleteComment")) === true) {
				await db.execute<ResultSetHeader>("DELETE FROM comments WHERE commentID = ? LIMIT 1", [commentID]);
			}
		}
