 ...
 */
const requestUserAccess = async (gdpsid: string, accountIDStr?: string, gjp2Str?: string, gjpStr?: string, req?: Request): Promise<string> => {
	try {
		...
		const requestModPerm = await ApiLib.getMaxValuePermission(gdpsid, accountID, "actionRequestMod");
		if (typeof requestModPerm === "number" && requestModPerm >= 1) {
			permState = await ApiLib.getMaxValuePermission(gdpsid, accountID, "modBadgeLevel");
