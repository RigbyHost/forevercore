...
const suggestStars = async (
	...
	try {
		...
		// Check permissions
		const starFeatured = await ApiLib.checkPermission(gdpsid, accountID, "actionRateFeature");
		const starEpic = await ApiLib.checkPermission(gdpsid, accountID, "actionRateEpic");
		const starLegendary = await ApiLib.checkPermission(gdpsid, accountID, "actionRateLegendary");
		const starMythic = await ApiLib.checkPermission(gdpsid, accountID, "actionRateMythic");

		// Handle based on permission levels
		if ((await ApiLib.checkPermission(gdpsid, accountID, "actionRateStars")) && !starFeatured && !starEpic && !starLegendary && !starMythic) {
			// Standard stars rater
			await ApiLib.rateLevel(gdpsid, accountID, levelID, parseInt(stars), difficulty.diff, difficulty.auto, difficulty.demon);
			...
			if (Number(feature) <= 1) {
				await ApiLib.rateLevel(gdpsid, accountID, levelID, parseInt(stars), difficulty.diff, difficulty.auto, difficulty.demon);
				await ApiLib.featureLevel(gdpsid, accountID, levelID, feature);
				await ApiLib.verifyCoinsLevel(gdpsid, accountID, levelID, 1);
				calculate(gdpsid);
		...
		} else if (await ApiLib.checkPermission(gdpsid, accountID, "actionRateMythic")) {
			// Mythic rater
			if (Number(feature) <= 4) {
				await ApiLib.rateLevel(gdpsid, accountID, levelID, parseInt(stars), difficulty.diff, difficulty.auto, difficulty.demon);
				...
				await ApiLib.featureLevel(gdpsid, accountIDS, levelIDS, featureS);
				await ApiLib.verifyCoinsLevel(gdpsid, accountID, levelID, 1);
				calculate(gdpsid);
			...
			await ApiLib.suggestLevel(gdpsid, accountID, levelID, difficulty.diff, stars, feature, difficulty.auto, difficulty.demon);
