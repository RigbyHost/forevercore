 ...
 */
const reportLevel = async (gdpsid: string, levelIDStr?: string, req?: Request): Promise<string> => {
	try {
		...
		const [rows] = await db.execute<RowDataPacket[]>("SELECT COUNT(*) AS count FROM reports WHERE levelID = ? AND hostname = ?", [levelID, ip]);

		if (rows[0].count == 0) {
			// Submit new report
			const [result] = await db.execute<ResultSetHeader>("INSERT INTO reports (levelID, hostname) VALUES (?, ?)", [levelID, ip]);
