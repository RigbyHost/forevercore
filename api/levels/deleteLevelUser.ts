...
const deleteLevelUser = async (
	...
	try {
		...
		const [result] = await db.execute<ResultSetHeader>("DELETE from levels WHERE levelID = ? AND userID = ? AND starStars = 0 LIMIT 1", [
			levelID,
			userID
		]);
		...
		const levelPath = path.join(__dirname, "..", "..", "GDPS_DATA", gdpsid, "data", "levels", `level_${levelID}.dat`);
		const deletedLevelPath = path.join(__dirname, "..", "..", "GDPS_DATA", gdpsid, "data", "levels", "deleted", `level_${levelID}.dat`);
