...
const updateDesc = async (
  ...
  try {
    ...
    if (udidStr) {
      ...
      id = await GJPCheck.getAccountIDOrDie(accountIDStr, gjp2Str, gjpStr, req);
    }
    ...
    await db.execute<ResultSetHeader>(
      ...
      [levelDesc, levelID, id]
    );
    
    ConsoleApi.Log("main", `Updated level desc: ${levelID}`);
    return "1";
  } catch (error) {
    ConsoleApi.Error("main", `${error} at net.fimastgd.forevercore.api.levels.updateDesc`);
    return "-1";
  }
};
