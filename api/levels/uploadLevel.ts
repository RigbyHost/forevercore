...
// import GJPCheck from '../lib/GJPCheck'; - unused import
...
const uploadLevel = async (
	...
	try {
		...
		if (getSettings(gdpsid).objectLimitFU === true) {
			if (getSettings(gdpsid).objectLimitCount < 5 || getSettings(gdpsid).objectLimitCount > 500) {
				ConsoleApi.FatalError("main", "Config error: getSettings(<string>gdpsid).objectLimitCount must be more than 5 and less than 500");
				return "-1";
			}

			if (parseInt(objects) < getSettings(gdpsid).objectLimitCount) {
		...
		// Get user ID
		let id = await ApiLib.getIDFromPost(gdpsid, udidStr, gameVersionStr, accountIDStr, gjp2Str, gjpStr, req);
		let hostname = await FixIp.getIP(req);
		let userID = await ApiLib.getUserID(gdpsid, id, userName);
		...
		if (levelString !== "" && levelName !== "") {
			...
			const [rowse] = await db.query<RowDataPacket[]>("SELECT levelID FROM levels WHERE levelName = ? AND userID = ?", [levelName, userID]);
			...
			if (rowse.length === 1) {
				...
				const lvlPath = path.join(__root, "GDPS_DATA", gdpsid, "data", "levels", `level_${levelID}.dat`);
				...
				ConsoleApi.Log("main", `Level ${levelName} (level_${levelID}.dat) uploaded`);
				return levelID;
			}
		} else {
		...
		ConsoleApi.Error("main", `${IOException} at net.fimastgd.forevercore.api.levels.uploadLevel`);
		return "-1";
	}
};
