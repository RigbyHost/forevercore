...
const rateDemon = async (
	...
	try {
		...
		const id = await GJPCheck.getAccountIDOrDie(gdpsid, accountIDStr, gjp2Str, gjpStr, req);

		// Check if user has permission to rate demons
		if (!(await ApiLib.checkPermission(gdpsid, id, "actionRateDemon"))) {
			return "-1";
		}
		...
		await db.execute<ResultSetHeader>("UPDATE levels SET starDemonDiff = ? WHERE levelID = ?", [dmn, levelID]);

		await db.execute<ResultSetHeader>("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (?, ?, ?, ?, ?)", [
			10,
			dmnname,
			levelID,
			timestamp,
