...
const deleteMessages = async (
	...
	try {
		...
		if (messagesStr) {
			...
			await db.query<ResultSetHeader>(`DELETE FROM messages WHERE messageID IN (${messages}) AND accID = ? LIMIT 10`, [accountID]);

			// Delete messages received by the user
			await db.query<ResultSetHeader>(`DELETE FROM messages WHERE messageID IN (${messages}) AND toAccountID = ? LIMIT 10`, [accountID]);

			ConsoleApi.Log("main", `Messages deleted`);
			return "1";
		} else {
			// Delete a single message
			await db.query<ResultSetHeader>("DELETE FROM messages WHERE messageID = ? AND accID = ? LIMIT 1", [messageID, accountID]);

			await db.query<ResultSetHeader>("DELETE FROM messages WHERE messageID = ? AND toAccountID = ? LIMIT 1", [messageID, accountID]);
