...
const downloadMessage = async (
	...
	try {
		...
		let accountID = await GJPCheck.getAccountIDOrDie(gdpsid, accountIDStr, gjp2Str, gjpStr, req);
       ...
       LIMIT 1`,
		...
		if (!isSenderStr) {
			...
			await db.query<ResultSetHeader>("UPDATE messages SET isNew=1 WHERE messageID = ? AND toAccountID = ?", [messageID, accountID]);
		...
		// Get user data
		const [users] = await db.query<UserData[]>("SELECT userName, userID, extID FROM users WHERE extID = ?", [accountID]);
