...
const getMessages = async (
	...
	try {
		...
		let msgstring = "";
		const toAccountID = await GJPCheck.getAccountIDOrDie(gdpsid, accountIDStr, gjp2Str, gjpStr, req);
		...
		for (const message1 of result) {
			if (message1.messageID !== undefined) {
				...
				// Get user data
				const [userResult] = await db.query<UserData[]>("SELECT * FROM users WHERE extID = ?", [accountID]);
