...
class CommandLib {
	...
	static async ownCommand(
		...
		const commandInComment: string = `!${command.toLowerCase()}`;
		const commandInPerms: string = command.charAt(0).toUpperCase() + command.slice(1).toLowerCase();
		const commandlength = commandInComment.length;

		// Check if comment starts with command and user has permission
		if (
			comment.substr(0, commandlength) == commandInComment &&
			((await ApiLib.checkPermission(gdpsid, accountID, `command${commandInPerms}All`)) ||
				(targetExtID == accountID && (await ApiLib.checkPermission(gdpsid, accountID, `command${commandInPerms}Own`))))
		) {
			return true;
		}
		return false;
	 ...
	 */
	static async doCommands(gdpsid: string, accountID: number | string, comment: string, levelID: number | string): Promise<boolean | string> {
		const db = await threadConnection(gdpsid);
		// Validate input
		...
		if (Number(levelID) < 0) {
			return await this.doListCommands(gdpsid, accountID, comment, levelID);
		}
		...
		// Get level owner
		const [rows] = await db.execute<RowDataPacket[]>("SELECT extID FROM levels WHERE levelID = ?", [levelID]);
		const targetExtID = rows[0]?.extID;

		// Process admin commands

		// !rate command - rate a level with stars, difficulty, etc.
		if (comment.startsWith("!rate") && (await ApiLib.checkPermission(gdpsid, accountID, "commandRate"))) {
			...
			await db.execute("INSERT INTO modactions (type, value, value2, value3, timestamp, account) VALUES (?, ?, ?, ?, ?, ?)", [
				1,
				commentarray[1],
				starStars,
				levelID,
			...
			if (starFeatured) {
				await db.execute("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (?, ?, ?, ?, ?)", [
					2,
					starFeatured,
					levelID,
					uploadDate,
			...
			calculate(gdpsid);
			return true;
		} else if (comment.startsWith("!rate") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandRate"))) {
			return "NO";
		}

		// !unrate command - remove rating from a level
		if (comment.startsWith("!unrate") && (await ApiLib.checkPermission(gdpsid, accountID, "commandUnrate"))) {
			let starStars = 0;
			const starCoins = 0;
		...
		if (comment.startsWith("!feature") && (await ApiLib.checkPermission(gdpsid, accountID, "commandFeature"))) {
			...
			return true;
		} else if (comment.startsWith("!feature") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandFeature"))) {
			return "NO";
		}

		// !epic command - set epic status
		if (comment.startsWith("!epic") && (await ApiLib.checkPermission(gdpsid, accountID, "commandEpic"))) {
			...
			return true;
		} else if (comment.startsWith("!epic") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandEpic"))) {
			return "NO";
		}

		// !unepic command - remove epic status
		if (comment.startsWith("!unepic") && (await ApiLib.checkPermission(gdpsid, accountID, "commandUnepic"))) {
			...
			return true;
		} else if (comment.startsWith("!unepic") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandUnepic"))) {
			return "NO";
		}

		// !verifycoins command - verify coins in level
		if (comment.startsWith("!verifycoins") && (await ApiLib.checkPermission(gdpsid, accountID, "commandVerifycoins"))) {
			...
			return true;
		} else if (comment.startsWith("!verifycoins") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandVerifycoins"))) {
			return "NO";
		}

		// !unverifycoins command - unverify coins in level
		if (comment.startsWith("!unverifycoins") && (await ApiLib.checkPermission(gdpsid, accountID, "commandVerifycoins"))) {
			...
			return true;
		} else if (comment.startsWith("!unverifycoins") && (await ApiLib.checkPermission(gdpsid, accountID, "commandUnverifycoins"))) {
			return "NO";
		}

		// !daily command - set level as daily
		if (comment.startsWith("!daily") && (await ApiLib.checkPermission(gdpsid, accountID, "commandDaily"))) {
			// Check if level is already a daily
			const [dailyCount] = await db.execute<RowDataPacket[]>("SELECT count(*) as count FROM dailyfeatures WHERE levelID = ? AND type = 0", [
				levelID
			]);
			...
			// Log action
			await db.execute("INSERT INTO modactions (type, value, value3, timestamp, account, value2, value4) VALUES (?, ?, ?, ?, ?, ?, ?)", [
				5,
				"1",
				levelID,
				uploadDate,
			...
			return true;
		} else if (comment.startsWith("!daily") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandDaily"))) {
			return "NO";
		}

		// !weekly command - set level as weekly
		if (comment.startsWith("!weekly") && (await ApiLib.checkPermission(gdpsid, accountID, "commandWeekly"))) {
			// Check if level is already a weekly
			const [weeklyCount] = await db.execute<RowDataPacket[]>("SELECT count(*) as count FROM dailyfeatures WHERE levelID = ? AND type = 1", [
				levelID
			]);
			...
			return true;
		} else if (comment.startsWith("!weekly") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandWeekly"))) {
			return "NO";
		}

		// !delete command - delete level
		if (comment.startsWith("!delete") && (await ApiLib.checkPermission(gdpsid, accountID, "commandDelete"))) {
			...
			return true;
		} else if (comment.startsWith("!delete") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandDelete"))) {
			return "NO";
		}

		// !setacc command - change level owner
		if (comment.startsWith("!setacc") && (await ApiLib.checkPermission(gdpsid, accountID, "commandSetacc"))) {
			// Find account by username or ID
			const [accountRows] = await db.execute<RowDataPacket[]>("SELECT accountID FROM accounts WHERE userName = ? OR accountID = ? LIMIT 1", [
				commentarray[1],
				commentarray[1]
			]);
			...
			const [userRows] = await db.execute<RowDataPacket[]>("SELECT userID FROM users WHERE extID = ? LIMIT 1", [targetAcc]);
			...
			return true;
		} else if (comment.startsWith("!setacc") && !(await ApiLib.checkPermission(gdpsid, accountID, "commandSetacc"))) {
			return "NO";
		}
		...
		if (await this.ownCommand(gdpsid, comment, "rename", accountID, targetExtID)) {
			...
			await db.execute("INSERT INTO modactions (type, value, timestamp, account, value3) VALUES (?, ?, ?, ?, ?)", [
				8,
				name,
				uploadDate,
				accountID,
		...
		if (await this.ownCommand(gdpsid, comment, "pass", accountID, targetExtID)) {
		...
		if (await this.ownCommand(gdpsid, comment, "description", accountID, targetExtID)) {
	 ...
	 */
	static async doListCommands(gdpsid: string, accountID: number | string, command: string, listID: number | string): Promise<boolean> {
		const db = await threadConnection(gdpsid);
		...
		const carray: string[] = command.split(" ");

		switch (carray[0]) {
			// !rate or !r command - rate a list
			case "!r":
			case "!rate":
				const [getList] = await db.query<RowDataPacket[]>("SELECT * FROM lists WHERE listID = ?", [listID]);

				const reward = await ExploitPatch.number(carray[1]);
				let diff: any = await ExploitPatch.charclean(carray[2]);
				const featured = isNaN(Number(carray[3])) ? await ExploitPatch.number(carray[4]) : await ExploitPatch.number(carray[3]);

				let count = isNaN(Number(carray[3])) ? await ExploitPatch.number(carray[5]) : await ExploitPatch.number(carray[4]);
				...
				if (await ApiLib.checkPermission(gdpsid, accountID, "commandRate")) {
					// Direct rating with mod permissions
					await db.query("UPDATE lists SET starStars = ?, starDifficulty = ?, starFeatured = ?, countForReward = ? WHERE listID = ?", [
						reward,
						diff,
						featured,
						count,
					...
					await db.query("INSERT INTO modactions (type, value, value2, value3, timestamp, account) VALUES (30, ?, ?, ?, ?, ?)", [
						reward,
						diff,
						listID,
				...
				} else if (await ApiLib.checkPermission(gdpsid, accountID, "actionSuggestRating")) {
					...
					await db.query("INSERT INTO modactions (type, value, value2, value3, timestamp, account) VALUES (31, ?, ?, ?, ?, ?)", [
						reward,
						diff,
						listID,
						Math.floor(Date.now() / 1000),
			...
			case "!f":
			case "!feature":
				if (!(await ApiLib.checkPermission(gdpsid, accountID, "commandFeature"))) return false;

				if (!carray[1]) carray[1] = "1";

				await db.query("UPDATE lists SET starFeatured = ? WHERE listID = ?", [carray[1], listID]);

				await db.query("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (32, ?, ?, ?, ?)", [
					carray[1],
					listID,
					Math.floor(Date.now() / 1000),
					accountID
			...
			case "!un":
			case "!unlist":
				const accCheck = await ApiLib.getListOwner(gdpsid, listID);

				if (!(await ApiLib.checkPermission(gdpsid, accountID, "commandUnlistAll")) && accountID != accCheck) return false;

				if (!carray[1]) carray[1] = "1";

				await db.query("UPDATE lists SET unlisted = ? WHERE listID = ?", [carray[1], listID]);

				await db.query("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (33, ?, ?, ?, ?)", [
					carray[1],
					listID,
					Math.floor(Date.now() / 1000),
					accountID
			...
			case "!d":
			case "!delete":
				if (!(await ApiLib.checkPermission(gdpsid, accountID, "commandDelete"))) return false;

				await db.query("DELETE FROM lists WHERE listID = ?", [listID]);

				await db.query("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (34, 0, ?, ?, ?)", [
					listID,
					Math.floor(Date.now() / 1000),
					accountID
				]);
			...
			case "!acc":
			case "!setacc":
				if (!(await ApiLib.checkPermission(gdpsid, accountID, "commandSetacc"))) return false;
				...
				if (!isNaN(Number(carray[1]))) {
					...
					acc = await ApiLib.getAccountIDFromName(gdpsid, await ExploitPatch.charclean(carray[1]));
				}
				...
				await db.query("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (35, ?, ?, ?, ?)", [
					acc,
					listID,
					Math.floor(Date.now() / 1000),
					accountID
			...
			case "!re":
			case "!rename":
				const accCheckRename = await ApiLib.getListOwner(gdpsid, listID);

				if (!(await ApiLib.checkPermission(gdpsid, accountID, "commandRenameAll")) && accountID != accCheckRename) return false;

				carray.shift();
				const oldName = await ApiLib.getListName(gdpsid, listID);
				...
				await db.query("INSERT INTO modactions (type, value, value2, value3, timestamp, account) VALUES (36, ?, ?, ?, ?, ?)", [
					name,
					oldName,
					listID,
					Math.floor(Date.now() / 1000),
			...
			case "!desc":
			case "!description":
				const accCheckDesc = await ApiLib.getListOwner(gdpsid, listID);

				if (!(await ApiLib.checkPermission(gdpsid, accountID, "commandDescriptionAll")) && accountID != accCheckDesc) return false;
				...
				await db.query("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES (37, ?, ?, ?, ?)", [
					description,
					listID,
					Math.floor(Date.now() / 1000),
					accountID
