...
const is = new Numbers(); // now without 'await'
...
class ApiLib {
	 ...
	 */
	public static async getMaxValuePermission(gdpsid: string, accountID: number | string, permission: string): Promise<number | boolean> {
		if (isNaN(Number(accountID))) return false;
		const db = await threadConnection(gdpsid);
		let maxValue = 0;
		try {
			const [roleIDarray] = await db.execute<RowDataPacket[]>("SELECT roleID FROM roleassign WHERE accountID = ?", [accountID]);
	 ...
	 */
	public static async getIDFromPost(
		gdpsid: string,
		udidStr?: string,
		...
		try {
			if (gjpStr) {
				if (udidStr && gameVersionStr && parseInt(gameVersionStr) < 20 && (await getSettings(gdpsid)).unregisteredSubmissions) {
					...
					const id = await this.getAccountIDOrDie(gdpsid, accountIDStr, undefined, gjpStr, req);
					return id;
				} else {
					return "-1";
				}
				...
				if (udidStr && gameVersionStr && parseInt(gameVersionStr) < 20 && (await getSettings(gdpsid)).unregisteredSubmissions) {
					...
					const id = await this.getAccountIDOrDie(gdpsid, accountIDStr, gjp2Str, undefined, req);
					return id;
				} else {
					return "-1";
				}
	 ...
	 * @returns User ID
	 */
	public static async getUserID(gdpsid: string, extID: string | number, userName: string = "Undefined"): Promise<number | string> {
		const db = await threadConnection(gdpsid as string);
		try {
			const register = this.isNumeric(extID) ? 1 : 0;
			const [rows] = await db.execute<RowDataPacket[]>("SELECT userID FROM users WHERE extID = ?", [extID]);
	 ...
	 */
	public static async checkPermission(gdpsid: string, accountID: number | string, permission: string): Promise<boolean> {
		const db = await threadConnection(gdpsid);
		try {
			// Check if admin
			const [adminRows] = await db.execute<RowDataPacket[]>("SELECT isAdmin FROM accounts WHERE accountID = ?", [accountID]);
			if (adminRows.length > 0 && adminRows[0].isAdmin === 1) {
				return true;
			}

			// Check role permissions
			const [roleRows] = await db.execute<RowDataPacket[]>("SELECT roleID FROM roleassign WHERE accountID = ?", [accountID]);
			...
			const [defaultRows] = await db.execute<RowDataPacket[]>(`SELECT ${permission} FROM roles WHERE isDefault = 1`);
	 ...
	 */
	public static async getFriends(gdpsid: string, accountID: number | string): Promise<number[]> {
		const db = await threadConnection(gdpsid);
		try {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT person1, person2 FROM friendships WHERE person1 = ? OR person2 = ?", [
				accountID,
				accountID
			]);
	 ...
	 */
	public static async getListLevels(gdpsid: string, listID: number | string): Promise<string | null> {
		const db = await threadConnection(gdpsid);
		try {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT listlevels FROM lists WHERE listID = ?", [listID]);
	 ...
	 */
	public static async isFriends(gdpsid: string, accountID: number | string, targetAccountID: number | string): Promise<boolean> {
		const db = await threadConnection(gdpsid);
		try {
	 ...
	 */
	public static getSongString(song: any): string | false {
		...
		return `1~|~${song.ID}~|~2~|~${song.name.replace(/#/g, "")}~|~3~|~${song.authorID}~|~4~|~${song.authorName}~|~5~|~${
			song.size
		}~|~6~|~~|~10~|~${dl}~|~7~|~~|~8~|~1`;
	}
	 ...
	 */
	public static getUserString(userdata: any): string {
	 ...
	 */
	public static async getDiffFromStars(stars: number | string): Promise<{
		diff: number;
		auto: number;
		demon: number;
		name: string;
	...
	public static async rateLevel(
		...
		try {
			...
			await db.execute("UPDATE levels SET starDemon=?, starAuto=?, starDifficulty=?, starStars=?, rateDate=? WHERE levelID=?", [
				demon,
				auto,
				difficulty,
				stars,
	 ...
	 */
	public static async getListOwner(gdpsid: string, listID: number | string): Promise<number | null> {
		const db = await threadConnection(gdpsid);
		try {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT accountID FROM lists WHERE listID = ?", [listID]);
	 ...
	 * @returns Account ID
	 */
	public static async getAccountIDFromName(gdpsid: string, userName: string): Promise<number> {
		const db = await threadConnection(gdpsid);
		try {
	 ...
	 * @returns List name
	 */
	public static async getListName(gdpsid: string, listID: number | string): Promise<string | null> {
		const db = await threadConnection(gdpsid);
		try {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT listName FROM lists WHERE listID = ?", [listID]);
	 ...
	 */
	public static async getAccountCommentColor(gdpsid: string, accountID: number | string): Promise<string> {
		const db = await threadConnection(gdpsid);
		try {
			if (!is.Int(accountID)) return "255,255,255";
			...
			const [defaultRows] = await db.execute<RowDataPacket[]>("SELECT commentColor FROM roles WHERE isDefault = 1");
	 ...
	 */
	public static async getDiffFromName(nameStr: string): Promise<[number, number, number]> {
	 ...
	 * @param demon - Demon flag
	 */
	public static async suggestLevel(
		gdpsid: string,
		accountID: number | string,
		levelID: number | string,
		difficulty: number,
	 ...
	 */
	public static async verifyCoinsLevel(gdpsid: string, accountID: number | string, levelID: number | string, coins: number): Promise<void> {
		const db = await threadConnection(gdpsid);
		try {
			await db.execute("UPDATE levels SET starCoins=? WHERE levelID=?", [coins, levelID]);
	 ...
	 */
	public static async featureLevel(gdpsid: string, accountID: number | string, levelID: number | string, state: number | string): Promise<void> {
		const db = await threadConnection(gdpsid);
		try {
			let feature = 0;
			let epic = 0;
			...
			await db.execute("UPDATE levels SET starFeatured=?, starEpic=?, rateDate=? WHERE levelID=?", [
				feature,
				epic,
				Math.floor(Date.now() / 1000),
				levelID
	 ...
	 */
	public static async getAccountName(gdpsid: string, accountID: number | string): Promise<string | false> {
		const db = await threadConnection(gdpsid);
		try {
			if (isNaN(Number(accountID))) return false;
	 ...
	 */
	public static async getDifficulty(diff: number, auto: number, demon: number): Promise<string> {
		if (auto !== 0) {
			return "Auto";
		} else if (demon !== 0) {
			return "Demon";
	 ...
	 */
	public static async validateGJPOrDie(gdpsid: string, gjp: string, accountID: number | string, req?: Request): Promise<string | void> {
		const result = await this.check(gdpsid as string, gjp, accountID, req);
		if (result !== 1) {
			return "-1";
		}
	}
	 ...
	 */
	public static async validateGJP2OrDie(gdpsid: string, gjp2: string, accountID: number | string, req?: Request): Promise<string | void> {
	 ...
	 */
	public static async getAccountIDOrDie(gdpsid: string, accountIDStr?: string, gjp2Str?: string, gjpStr?: string, req?: Request): Promise<string> {
		if (!accountIDStr) {
			return "-1";
		}
	 ...
	 */
	public static async GJP2fromPassword(pass: string, req: Request = {} as Request): Promise<string> {
		return crypto
			.createHash("sha1")
	 ...
	 */
	public static async GJP2hash(pass: string, req: Request = {} as Request): Promise<string> {
	...
	public static async assignModIPs(gdpsid: string, accountID: number | string, ip: string, req?: Request): Promise<void> {
		const db = await threadConnection(gdpsid);
		const modipCategory = await this.getMaxValuePermission(gdpsid, accountID, "modipCategory");
	...
	public static async isGJP2Valid(gdpsid: string, accid: number | string, gjp2: string, req?: Request): Promise<number> {
		...
		if (isPasswordValid) {
			if (req) await this.assignModIPs(gdpsid, accid, FixIp.getIP(req), req);
			return userInfo.isActive ? 1 : -2;
		} else {
			if (req) await this.logInvalidAttemptFromIP(gdpsid, accid, req);
			return 0;
		}
	}
	 ...
	 */
	public static async assignGJP2(gdpsid: string, accid: number | string, pass: string, req?: Request): Promise<any> {
		const db = await threadConnection(gdpsid);
