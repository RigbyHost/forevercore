...
class GenerateHash {
	...
	static genSolo(levelstring: string): string {
		...
		if (len < 41) {
			return crypto
				.createHash("sha1")
				.update(levelstring + "xI25fpAapCQg")
				.digest("hex");
		}
	...
	static genSolo2(lvlsmultistring: string): string {
		return crypto
			.createHash("sha1")
			.update(lvlsmultistring + "xI25fpAapCQg")
			.digest("hex");
	}
	...
	static genSolo3(lvlsmultistring: string): string {
		return crypto
			.createHash("sha1")
			.update(lvlsmultistring + "oC36fpYaPtdg")
			.digest("hex");
	}
	...
	static genSolo4(lvlsmultistring: string): string {
		return crypto
			.createHash("sha1")
			.update(lvlsmultistring + "pC26fpYaQCtg")
			.digest("hex");
	}
	 ...
	 * @returns SHA1 hash
	 */
	static async genPack(gdpsid: string, lvlsmultistring: string): Promise<string> {
		...
		for (const id of lvlsarray) {
			...
			const [result2] = await db.execute<RowDataPacket[]>("SELECT stars, coins FROM mappacks WHERE ID = ?", [idParse]);
