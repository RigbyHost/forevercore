 ...
 */
const likeItem = async (gdpsid: string, typeStr?: string, likeStr?: string, itemIDStr?: string, req?: Request): Promise<string> => {
	try {
		...
		// Record like action
		await db.execute<ResultSetHeader>("INSERT INTO actions_likes (itemID, type, isLike, ip) VALUES (?, ?, ?, INET6_ATON(?))", [
			itemID,
			type,
			isLike,
			ip
		...
		// Get current likes
		const [likesResult] = await db.execute<RowDataPacket[]>(`SELECT likes FROM ${table} WHERE ${column} = ? LIMIT 1`, [itemID]);

		// Update likes count
		const sign = isLike == 1 ? "+" : "-";
		await db.execute<ResultSetHeader>(`UPDATE ${table} SET likes = likes ${sign} 1 WHERE ${column} = ?`, [itemID]);
