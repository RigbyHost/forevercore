 ...
 */
const getChallenges = async (gdpsid: string, accountIDStr?: string, udidStr?: string, chkStr?: string): Promise<string> => {
	try {
		...
		if (accountID !== "0") {
			userID = await ApiLib.getUserID(gdpsid, accountID);
		} else {
			userID = await ApiLib.getUserID(gdpsid, udid);
		}
