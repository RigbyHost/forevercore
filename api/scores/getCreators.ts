 ...
 */
const getCreators = async (gdpsid: string, accountIDStr?: string, typeStr?: string): Promise<string> => {
	try {
		...
		for (const user of result) {
			...
			pplstring += `1:${user.userName}:2:${user.userID}:13:${user.coins}:17:${user.userCoins}:6:${xi}:9:${user.icon}:10:${user.color1}:11:${
				user.color2
			}:14:${user.iconType}:15:${user.special}:16:${extid}:3:${user.stars}:8:${Math.floor(user.creatorPoints)}:4:${user.demons}:7:${extid}:46:${
				user.diamonds
			}|`;
		}
