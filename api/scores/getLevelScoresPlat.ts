 ...
 */
const getLevelScoresPlat = async (gdpsid: string, req: Request): Promise<string> => {
	try {
		const db = await threadConnection(gdpsid);
		// Authenticate user
		const accountID = parseInt(await GJPCheck.getAccountIDOrDie(gdpsid, req.body.accountID, req.body.gjp2, req.body.gjp, req));
		...
		// UPDATING SCORE
		const [oldScoreRows] = await db.query<RowDataPacket[]>(`SELECT ${mode} FROM platscores WHERE accountID = ? AND levelID = ?`, [
			accountID,
			levelID
		]);
		...
		if (oldScoreRows.length == 0) {
			if (scores.time > 0) {
				await db.query("INSERT INTO platscores (accountID, levelID, time, timestamp) VALUES (?, ?, ?, ?)", [
					accountID,
					levelID,
					scores.time,
					uploadDate
			...
			if (((mode == "time" && oldScore > scores.time) || (mode == "points" && oldScore < scores.points)) && scores.time > 0) {
				await db.query(`UPDATE platscores SET ${mode}=?, timestamp=? WHERE accountID=? AND levelID=?`, [
					scores[mode],
					uploadDate,
					accountID,
					levelID
		...
		switch (parseInt(type.toString())) {
			case 0: // Friends
				const friends = await ApiLib.getFriends(gdpsid, accountID);
