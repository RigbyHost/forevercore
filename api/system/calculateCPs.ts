 ...
 */
const calculate = async (gdpsid): Promise<string> => {
	...
	try {
		...
		for (const level of sharedLevels) {
			...
			const [shares] = await db.query<RowDataPacket[]>("SELECT userID FROM cpshares WHERE levelID = ?", [level.levelID]);
		...
		for (const [userID, cp] of Object.entries(people)) {
			await db.query("UPDATE users SET creatorPoints = (creatorpoints + ?) WHERE userID = ?", [cp, userID]);
		}
