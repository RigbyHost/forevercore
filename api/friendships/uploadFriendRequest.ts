...
const uploadFriendRequest = async (
	...
	try {
		...
		const [blocked] = await db.query<RowDataPacket[]>("SELECT ID FROM `blocks` WHERE person1 = ? AND person2 = ?", [toAccountID, accountID]);

		// Check if receiver has friends-only setting
		const [frSOnly] = await db.query<RowDataPacket[]>("SELECT frS FROM `accounts` WHERE accountID = ? AND frS = 1", [toAccountID]);
		...
		if (existingRequests[0].count == 0 && !blocked.length && !frSOnly.length) {
			await db.query<ResultSetHeader>("INSERT INTO friendreqs (accountID, toAccountID, comment, uploadDate) VALUES (?, ?, ?, ?)", [
				accountID,
				toAccountID,
				comment,
				uploadDate
			...
			ConsoleApi.Log(
				"main",
				`Failed to upload friend request to ${toAccountID} from ${accountID}: existingRequests[0].count equal '0' and blocked.length not found and frSOnly.length not found`
			);
			return "-1";
		}
	} catch (error) {
