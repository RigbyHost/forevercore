 ...
 */
const acceptFriendRequest = async (gdpsid: string, req: Request): Promise<string> => {
	try {
		...
		const [requests] = await db.query<FriendRequest[]>("SELECT accountID, toAccountID FROM friendreqs WHERE ID = ?", [requestID]);
		...
		await db.query<ResultSetHeader>("INSERT INTO friendships (person1, person2, isNew1, isNew2) VALUES (?, ?, 1, 1)", [
			reqAccountID,
			toAccountID
		]);

		await db.query<ResultSetHeader>("DELETE from friendreqs WHERE ID = ? LIMIT 1", [requestID]);
