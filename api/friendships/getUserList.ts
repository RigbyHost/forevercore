 ...
 */
const getUserList = async (gdpsid: string, req: Request): Promise<string> => {
	try {
		const db = await threadConnection(gdpsid);
		// Validate required parametersimport deleteFriendRequests from "../../api/friendships/deleteFriendRequests";
		...
		for (const user of users) {
			...
			peoplestring += `1:${user.userName}:2:${user.userID}:9:${user.icon}:10:${user.color1}:11:${user.color2}:14:${user.iconType}:15:${
				user.special
			}:16:${user.extID}:18:0:41:${newMap.get(user.extID)}|`;
		}
		...
		if (peoplestring == "") {
			ConsoleApi.Warn("main", `Failed to get user list: peoplestring is empty. accountID: ${accountID}`);
			return "-1";
		}
