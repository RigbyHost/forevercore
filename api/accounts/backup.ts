...
const backupAccount = async (
	...
	try {
		...
		if (!userNameOr && accountIDOr) {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT userName FROM accounts WHERE accountID = ?", [accountIDOr]);
		...
		const bufferData = Buffer.from(saveDataDecoded, "base64");
		saveDataDecoded = (await gunzip(bufferData)).toString();
		...
		if (userName) {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT extID FROM users WHERE userName = ? LIMIT 1", [userName]);
			extID = rows.length ? rows[0].extID : null;
		} else {
			const [rows] = await db.execute<RowDataPacket[]>("SELECT userName FROM users WHERE extID = ? LIMIT 1", [accountID]);
			userNameFin = rows.length ? rows[0].userName : null;
		}

		// update user stats
		if (extID) {
			await db.execute<RowDataPacket[]>("UPDATE users SET orbs = ?, completedLvls = ? WHERE extID = ?", [orbs, lvls, extID]);
		} else if (userNameFin) {
			await db.execute<RowDataPacket[]>("UPDATE users SET orbs = ?, completedLvls = ? WHERE userName = ?", [orbs, lvls, userNameFin]);
		}
