...
const getUserInfo = async (
  targetAccountIDStr: string | undefined,
  accountIDStr: string | undefined,
  gjp2Str: string | undefined,
  gjpStr: string | undefined,
  req?: Request
): Promise<string> => {
  try {
    let appendix = "";
    ...
    [rows] = await db.execute<RowDataPacket[]>(
      "SELECT count(*) as count FROM users WHERE stars > ? AND isBanned = 0", 
      [user.stars]
    );
    ...
    if (me === extid) {
      ...
      const [requests] = await db.execute<RowDataPacket[]>(
        "SELECT count(*) as count FROM friendreqs WHERE toAccountID = ?", 
        [me]
      );
      
      const [pms] = await db.execute<RowDataPacket[]>(
        "SELECT count(*) as count FROM messages WHERE toAccountID = ? AND isNew=0", 
        [me]
      );
      ...
      if (INCrequests.length > 0) {
        const uploaddate = new Date(INCrequests[0].uploadDate * 1000).toLocaleString(
          "en-GB", 
