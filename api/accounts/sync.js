...
const syncAccount = async (userNameStr, accountIDStr, passwordStr, gjp2Str, req) => {
    try {
    function dateNow() {
    ...
    // STOP using fs.writeFile('../'),
    // USE 'path'!
    const accountsPath = await path.join("./data/accounts", `${accountIDStr}.dat`);
    const accountsKeyPath = await path.join("./data/accounts/keys", `${accountIDStr}`);
