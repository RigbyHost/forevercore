 ...
 */
const uploadList = async (gdpsid: string, req: Request): Promise<string> => {
	try {
		...
		const listName =
			req.body.listName && (await (ExploitPatch.remove(req.body.listName) != ""))
		...
		const listVersion = parseInt(ExploitPatch.number(req.body.listVersion)) === 0 ? 1 : await ExploitPatch.number(req.body.listVersion);
		...
		if (listID !== 0) {
			const [rows] = await db.query<RowDataPacket[]>("SELECT * FROM lists WHERE listID = ? AND accountID = ?", [listID, accountID]);
