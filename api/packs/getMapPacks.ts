 ...
 */
const getMapPacks = async (gdpsid: string, req: Request): Promise<string> => {
	try {
		...
		const response = `${mappackstring}#${totalpackcount}:${packpage}:10#${await GenerateHash.genPack(gdpsid, lvlsmultistring)}`;
